...
test('passthrough middleware when action is not a function', (assert) => {
  ...
  codi()(store)(next)(action);
  assert.ok(next.calledWith(action), 'calls next with action');
  assert.end();
});


test('thunk with dependency injection', async (assert) => {
  ...
  const result = await codi(getConfig)(store)(next)(action);
...
test('generator with dependency injection', async (assert) => {
  const store = { dispatch: sinon.spy(), getState: sinon.spy() };
  const config = { aValue: true };
  const getConfig = sinon.spy(({ dispatch, getState }) => {
    getState('getConfig');
  ...
  const action = sinon.spy(function *({ dispatch, getState }) {
    yield Promise.resolve(getState('action'));
    yield Promise.resolve(dispatch('action'));
    return 'actionResult';
  });
  const next = sinon.stub();
